...
function draw() {
  ...
  if(gameState === PLAY){
    ...
    score = score + Math.ceil((frameCount/frameRate())/100);
    
    //creating infinite ground
    ground.velocityX = -(10 + score/60);
    if(ground.x <= 240){ 
       ground.x = 300;
    }
  ...
  fill("black");
  textSize(20);
  text("Survival Time: " + score,400,y);
}

//creating obstacles
function createObstacles(){
 if (frameCount % 100 === 0){
   obstacle = createSprite(600,220,10,40);
   obstacle.velocityX = -(10 + score/60);
...
function createBanana(){
 if (frameCount % 80 === 0){
   banana = createSprite(600,70,10,40);
   banana.velocityX = -(10 + score/60);
